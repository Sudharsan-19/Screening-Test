let FunctionProgressBar = () =>{
    var TotalQuestions = JSON.parse(localStorage.getItem('ParentsScreeningQuestions')).length;
    var ProgressBarWidthPercentage = (((Number($('#Question').attr('QuestionIndex'))) / 15) * 100) + "%";
    if(parseFloat(ProgressBarWidthPercentage) >= 100){
        location.href = 'Temp.html';
        localStorage.removeItem('ParentsScreeningQuestions')
...
let ReloadQuestion = async (PrevNext) =>{
    ...
    if(localStorage.getItem('ParentsScreeningQuestions') === null){
        var Questions = await AjaxScripts('FetchParentScreeningQuestions', { ChildDetails:ChildDatas }, 'FetchParentScreeningQuestions')
        localStorage.setItem('ParentsScreeningQuestions', JSON.stringify(Questions.data))
        $('#Question').text(Questions.data[Questions.NextStep]).attr('QuestionIndex', Questions.NextStep)
    }
    else{
        Questions = { data:JSON.parse(localStorage.getItem('ParentsScreeningQuestions')) };
...
let LoadAudioToLanguage = () =>{
    ...
    var audio = $("audio");
    $("audio").attr("src", "https://translate.google.com/translate_tts?ie=UTF-&&client=tw-ob&tl="+($('select').val() || 'en')+"&q="+$('#Question').text().replace('?',''));
    /****************/
    audio[0].pause();
...
$('#NextButton').on('click', async ()=>{
    ...
    if(Number($('#Question').attr('QuestionIndex')) + 1 > JSON.parse(localStorage.getItem('ParentsScreeningQuestions')).length){
        var CallBack = () =>{ 
