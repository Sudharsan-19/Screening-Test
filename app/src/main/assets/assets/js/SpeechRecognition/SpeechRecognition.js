...
if (!('webkitSpeechRecognition' in window)) {
    ...
    recognition.onstart = function () {
        ...
        talk_button.innerHTML = '<i class="bi bi-mic" style="font-size: 20px;"></i></i>Stop Recording';
    };
    ...
    recognition.onend = function () {
        ...
        if (!final_transcript) {
            ...
            talk_button.innerHTML = '<i class="bi bi-mic" style="font-size: 20px;"></i></i> Start Recording';
            return;
        }
    };
...
function talkWithApp(event) {
    ...
    recognition.start();
    $(talk_button).find('i').text('').addClass('bi-mic');
    ignore_onend = false;
...
function speechMyText(textToSpeech) {
    $(talk_button).find('i').text('').removeClass('bi-mic-mute');
